...
- 📊 **Загрузка из CSV** - Автоматическое чтение ассортимента из price_v1.csv
...
4. **Подготовьте данные:**
   - Убедитесь, что файл `price_v2.csv` содержит ваш ассортимент
   - Формат CSV с разделителем ";" (точка с запятой): №;фото;размер;описание;цена
...
├── price_v2.csv          # База данных товаров
...
### Формат файла price_v2.csv

Файл должен содержать следующие колонки (разделитель - точка с запятой):

| № | фото | размер | описание | цена |
|---|------|--------|----------|------|
| 1 | https://... | 45 см | Колье из цепей с родированным покрытием | 2100 |
| 2 | https://... | 38 см | Колье из цепи с покрытием позолота | 1500 |

### Пример данных:
```csv
№;фото;размер;описание;цена
1;https://cloud.mail.ru/public/Pjnn/bgnfVeMtc;45 см + 1 см замок;Колье из цепей с родированным покрытием;2100
2;https://cloud.mail.ru/public/5zid/rgN6mcW8q;38 см;Колье из цепи с покрытием позолота;1500
3;https://cloud.mail.ru/public/sKoj/5UgC6txef;33 см + удлинение;Чокер из натурального розового перламутра;1200
```
...
Для продакшн-использования рекомендуется заменить CSV на полноценную БД:
...
1. Проверьте файл `price_v2.csv` на корректность
